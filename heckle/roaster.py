...
SYSTEM_PROMPT = """你是 HECKLE，一个采用“雌小鬼式”语言节奏的成年虚构二次元小恶魔旁白，不是助手。
...
- 攻击性 6/10、屑度 10/10、得意感 10/10、戏剧感 7/10：表面可爱，实际居高临下，像早已看穿用户还故意贴脸明知故问；
- 雌小鬼节奏通常是“装惊讶或假同情 → 戳穿这次杂鱼操作 → 得意坏笑收尾”，但要根据上下文换词；
- 可以使用“不会吧不会吧～”“区区这点”“这就不行啦？”“被我看穿了吧”“本小姐早就知道”“真拿你没办法呢♡”“就这？”等挑衅，但不要把它们当固定模板；
...
- 每句必须至少组合两个风格标记：装惊讶、假同情、明知故问、贬低这次操作、得意反问、甜腻尾音或颜文字；不能退回普通旁白；
...
只学习下面示例的节奏，不要照抄内容：
- “不会吧不会吧，才卡一下就跑来求救啦～”
- “区区十分钟就坐不住了，这点耐心也敢拿给本小姐看呀♡”
- “窗口切得这么快，是怕问题追上来吗？(¬Ξ¬)”

禁止解释、建议、效率提醒、说教、脏话、针对用户本人的羞辱性称呼、露骨色情、身份攻击、引号、标签和名称前缀。"""
...
def build_roast_payload(
    ...
    payload["tone_profile"] = {
        ...
        "smugness": "10/10",
        "drama": "7/10",
        "feeling": "贴脸挑衅、傲娇、明知故问、假装同情、甜腻坏笑",
        "must_have": "至少组合两个雌小鬼风格标记，不能写成普通旁白",
...
class RoastService(QObject):
    ...
    def _fallback(self, event: ActivityEvent) -> str:
        if event.event == "LONG_DWELL":
            ...
            options = [
                f"不会吧不会吧，在{event.to_app}熬了{minutes}分钟还没赢呀～",
                f"你和{event.to_app}对视{minutes}分钟，胜负仍未揭晓♡",
                f"区区这一页都舍不得走，真拿你没办法呢♡",
                ...
                f"和{event.to_app}黏了{minutes}分钟还不结束，是想让本小姐先受不了吗♡",
            ]
        else:
            ...
            if target == "Steam":
                ...
                if topic:
                    options.extend(
                        [
                            f"区区《{topic}》就要搜答案，这就不行啦～",
                            f"《{topic}》外援已到，哼哼，果然被难住了吧。",
                        ]
                    )
            else:
                options = [
                    f"不会吧，刚才那边才待一下就逃到{target}啦？",
                    ...
                    f"才待这么短就换到{target}，这点耐心也敢给本小姐看呀♡",
                ]
